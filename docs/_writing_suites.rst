...
following variant of the previous ``tool_init`` command (the only difference is now we are adding the ``--macros`` flag).
...
- Implementation details - `Pull Request #129 <https://bitbucket.org/galaxy/galaxy-central/pull-request/129/implement-macro-engine-to-reduce-tool/diff>`__ and `Pull Request #140 <https://bitbucket.org/galaxy/galaxy-central/pull-request/140/improvements-to-tool-xml-macroing-system/diff>`__
