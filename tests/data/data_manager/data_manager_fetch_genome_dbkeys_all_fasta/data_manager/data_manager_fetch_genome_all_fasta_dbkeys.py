...
def add_fasta_to_table(
    data_manager_dict, fasta_readers, target_directory, dbkey, dbkey_name, sequence_id, sequence_name, params, **kwds
):
    fasta_filename = _stream_fasta_to_file(
        fasta_stream=fasta_readers, target_directory=target_directory, sequence_id=sequence_id
    )
    sort_fasta(fasta_filename, params["param_dict"]["sorting"]["sort_selector"], params)
    if dbkey_name:
        add_dbkey_to_table(
            data_manager_dict=data_manager_dict,
            target_directory=target_directory,
            dbkey=dbkey,
            dbkey_name=dbkey_name,
            fasta_filename=fasta_filename,
        )
    _add_data_table_entry(
        data_manager_dict,
        data_table_entry=dict(
            value=sequence_id, dbkey=dbkey, name=sequence_name, path=os.path.basename(fasta_filename)
        ),
...
def add_dbkey_to_table(data_manager_dict, target_directory, dbkey, dbkey_name, fasta_filename):
    ...
    _add_data_table_entry(data_manager_dict, data_table_entry=dbkey_dict, data_table_name="__dbkeys__")
...
def copy_from_directory(
    data_manager_dict, params, target_directory, dbkey, dbkey_name, sequence_id, sequence_name, tmp_dir
):
    ...
    if create_symlink:
        data_table_entries = _create_symlink(
            input_filename, target_directory, dbkey, dbkey_name, sequence_id, sequence_name
        )
    else:
        if isinstance(input_filename, list):
            fasta_readers = [get_stream_reader(open(filename, "rb"), tmp_dir) for filename in input_filename]
...
def _create_symlink(input_filename, target_directory, dbkey, dbkey_name, sequence_id, sequence_name):
    ...
    return [
        ("__dbkeys__", dbkey_dict),
        ("all_fasta", dict(value=sequence_id, dbkey=dbkey, name=sequence_name, path=fasta_base_filename)),
    ]


REFERENCE_SOURCE_TO_DOWNLOAD = dict(
    ucsc=download_from_ucsc,
    ncbi=download_from_ncbi,
...
SORTING_METHODS = dict(
    ...
    lexicographical=_sort_fasta_lexicographical,
    gatk=_sort_fasta_gatk,
    custom=_sort_fasta_custom,
)
...
def main():
    ...
    dbkey, dbkey_name, sequence_id, sequence_name = get_dbkey_dbname_id_name(
        params, dbkey_description=options.dbkey_description
    )
    ...
    try:
        ...
        fasta_readers = REFERENCE_SOURCE_TO_DOWNLOAD[reference_source](
            data_manager_dict=data_manager_dict,
            params=params,
            target_directory=target_directory,
            dbkey=dbkey,
